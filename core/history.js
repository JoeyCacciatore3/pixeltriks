...
GF.history = (function () {
  ...
  function undo(doc) {
    ...
    redoStack.push({ label: top.label, snap: snapshot(doc) }); // carry the step name forward
    restore(doc, top.snap);
    emit();
    return true;
  }
  ...
  function redo(doc) {
    ...
    undoStack.push({ label: top.label, snap: snapshot(doc) });
    restore(doc, top.snap);
    emit();
    return true;
  }
  ...
  /** Timeline for a history panel: `undo` oldest→newest (steps you can step back
      through), `redo` next-to-redo first (steps you can step forward through). */
  function info() {
    return { undo: undoStack.map(e => e.label), redo: redoStack.slice().reverse().map(e => e.label) };
  }

  return { push, undo, redo, clear, canUndo, canRedo, onChange, info };
})();
