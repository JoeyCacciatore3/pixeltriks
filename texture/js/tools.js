...
GF.view = (function () {
  ...
  function render() {
    ...
    // selection marching-ants (cached boundary canvas, document space)
    if (GF.select && GF.select.has()) {
      ctx.save();
      ctx.imageSmoothingEnabled = false;
      ctx.drawImage(GF.select.vizCanvas(antsPhase), 0, 0);
      ctx.restore();
    }
  ...
  function onDown(e) {
    ...
    if (tool === 'move') {
      ...
      if (tool === 'wand') {
        ...
        GF.select.wand(img, x, y, view.wand.tolerance, view.wand.contiguous, mode);
        const n = GF.select.count();
        U.toast(n ? n.toLocaleString() + ' px selected' : 'Nothing selected');
      } else {
      ...
      selDrag = { x0: p.x, y0: p.y, x1: p.x, y1: p.y, pts: [p],
